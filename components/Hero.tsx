...
const Hero = () => {
    ...
    return (
        <main className=' max-container w-full border-b-2 border-[var(--border)]'>
            <div className='padding-container flex  xs:flex-col lg:flex-row  justify-between   '>
                            ...
                            {socialLinks.map((link, index) => (
                                <li key={index}
                                    ...
                                    <a href={link.href} target='_blank' className='flex items-center  ' title='anchor to go to icon'>
                                        <span className=' flex-center m-2 text-xl w-[40px] h-[40px] text-black bg-[var(--icon-theme-color)] font-bold rounded-full '>{link.icon}</span>
